...
namespace Ironside\Controllers;

use App\Models\NavigationDashboard;
use Ironisde\Controllers\Traits\CRUDNotify;
...
//        $this->middleware('role:' . $this->selectedNavigation->id);
    ...
    protected function getBreadCrumb()
    {
        ...
        if (count($navigation) == 1 && $navigation[0]->title == 'Dashboard') {
            $html .= '<li><a href="' . $url . '"><i class="fa fa-home"></i>'. __("dashboard.dashboard") .'</a></li>';
        }
        else {
            foreach ($navigation as $key => $nav) {
    ...
    {
        $navigation = $this->urlParentNavs;
        $html = '<h2 class="text-primary">';

        // for dashboard, only add home
        if (count($navigation) == 1 && $navigation[0]->title == 'Dashboard') {
            $html .= '<i class="fa fa-home"></i> ' . __("dashboard.dashboard");
        }
        else {
            //foreach ($navigation as $key => $nav) {
            //    $html .= '<li>';
