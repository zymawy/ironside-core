...
var getGradientArray = (paramCount) => {
    paramCount -= 1;
    if (0 <= paramCount && paramCount < coolhueData.length) {
        return coolhueData[paramCount];
    } else {
        return "Pass an argument between 1 to " + coolhueData.length;
    }
}

var getGradientStyle = (paramCount) => {
    paramCount -= 1;
    if (0 <= paramCount && paramCount < coolhueData.length) {
        return gradientType + coolhueData[paramCount][0] + gradientStart + coolhueData[paramCount][1] + gradientEnd;;
    } else {
        return "Pass an argument between 1 to " + coolhueData.length;
    }
}

module.exports.getGradientArray = getGradientArray;
module.exports.getGradientStyle = getGradientStyle;