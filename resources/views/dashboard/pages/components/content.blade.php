...
@section('content')
        ...
        <div class="col-xs-12">
            <div class="card card-primary box-solid">
                ...
                <div class="card-body no-padding">
                    ...
                    <form method="POST" action="/dashboard/pages/{{ $page->id . '/sections/content' . (isset($item)? "/{$item->id}" : '')}}" accept-charset="UTF-8" enctype="multipart/form-data">
                        <input name="_token" type="hidden" value="{{ csrf_token() }}">
                        <input name="page_id" type="hidden" value="{{ $page->id }}">
                        <input name="_method" type="hidden" value="{{isset($item)? 'PUT':'POST'}}">

                        <fieldset>
                            @include('ironside::dashboard.pages.components.form_heading')
                            ...
                            @include('ironside::dashboard.pages.components.form_content')
                        ...
                        {{--@include('ironside::admin.partials.form_footer')--}}
                        <div class="form-footer">
                            ...
                            <a href="/dashboard/pages/{{ $page->id }}/edit" class="btn btn-labeled btn-default">
    ...
    @if(isset($item))
        @include('ironside::dashboard.photos.photoable', ['photoable' => $item, 'photos' => $item->photos])

        @include('ironside::dashboard.documents.documentable', ['documentable' => $item, 'documents' => $item->documents])
    @endif
@endsection
